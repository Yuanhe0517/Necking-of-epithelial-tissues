...
namespace VMTutorial
{
	void Dump::dump_cells(const string &vtk_file, bool binary_output, bool draw_periodic)
	{
		...
		vtkSmartPointer<vtkDoubleArray> energys = vtkSmartPointer<vtkDoubleArray>::New();
		vtkSmartPointer<vtkDoubleArray> stress_xx = vtkSmartPointer<vtkDoubleArray>::New();
		vtkSmartPointer<vtkDoubleArray> stress_yy = vtkSmartPointer<vtkDoubleArray>::New();
		...
		energys->SetName("cell energy");
		energys->SetNumberOfComponents(1);
		stress_xx->SetName("StressXX");
		stress_xx->SetNumberOfComponents(1);
		stress_yy->SetName("StressYY");
		stress_yy->SetNumberOfComponents(1);
		stress_xy->SetName("StressXY");
		stress_xy->SetNumberOfComponents(1);
		...
		for (auto f : _sys.mesh().faces())
		{
			if (!f.erased)
			{
				...
				if (!(f.outer || omit))
				{
					...
					energys->InsertNextValue(_force_compute.energy(f));
					stress_xx->InsertNextValue(this->get_face_stress_xx(f));
					stress_yy->InsertNextValue(this->get_face_stress_yy(f));
		...
		polydata->GetCellData()->AddArray(energys);
		polydata->GetCellData()->AddArray(stress_xx);
		polydata->GetCellData()->AddArray(stress_yy);
