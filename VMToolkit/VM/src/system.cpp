...
namespace VMTutorial
{
  ...
  void System::read_input(const string& json_file)
  {
    ...
    for (int i = 0; i < j["mesh"]["vertices"].size(); i++)
    {
      ...
      //cout<<{j["mesh"]["vertices"][i]["type"]<<endl
      //cout<<v.data().vert_type<<endl;
      //cout<< v.data().vert_type<< j["mesh"]["vertices"][i]["type"]<< endl;
    ...
    // {
    //   cout<<_mesh.vertices()[i].id<<";"<<_mesh.vertices()[i]._he<<endl;//<<";"<<_mesh.vertices()[i].data()<<endl;
    // } 
    //cout<<_mesh._halfedges.begin()<<endl;

    // 使用std::unordered_set存储需要判断的e_idx值
    //std::unordered_set<int> idx_set = {30, 80, 102, 140, 190, 200, 240,280, 300,340,380,430, 452, 500,540,580, 600, 649,701, 740,800,860,900,930};
    //std::unordered_set<int> idx_set = {67,90,111,195,218,239,323,346,367,451,474,495, 579,602,623,707,730,751,835,858,879};
    // std::unordered_set<int> idx_set = {30, 80, 102, 140, 190, 200, 240,278, 298,338,379,428, 451, 499,539,579, 599, 649,701, 740,800,860,898,930};
    // for (auto e : _mesh.edges())
    // {
    //   //cout<<e._idx<<";"<<e.i<<";"<<e.j<<endl;
    //   if (idx_set.find(e._idx) != idx_set.end())//(e.he()->from()->id == 405 && e.he()->to()->id == 379 ) 
    //     { _mesh.T1(e, 0.3);
    //       e.he()->from()->data().constraint = "defect";
    //       e.he()->to()->data().constraint = "defect";
    //       cout<<e.he()->from()->data().constraint <<endl;
    //       cout<<e.he()->to()->data().constraint <<endl;
